...
const numbDict: { [key: string]: string } = {
    zero: '0',
    one: '1',
    two: '2',
    three: '3',
...
for (let val of values) {
    for (const letter of Object.keys(numbDict)) {
        const firstCharOfReplacement = letter[0];
        const lastCharOfReplacement = letter.slice(-1);
        val = val.replace(
            new RegExp(letter, 'g'),
            firstCharOfReplacement + numbDict[letter] + lastCharOfReplacement,
        );
    }

    // Regex get numbers from value
    const regex = /\d+/g;
    const found = val.match(regex);
    if (!found) continue;

    const foundNumbers = found.join('');
    const number = parseInt(foundNumbers[0] + foundNumbers.slice(-1));
